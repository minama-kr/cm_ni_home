...
<body id="mng">
	<form id="frm" action="/mng/contents/hospital_album.asp" method="post">
		<!-- 상단 메뉴 -->
		...
		<div style="width:958px; margin:0 auto; overflow:hidden;">
			...
			<!-- 서브 페이지 컨텐츠 입력 -->
				...
				<div class="right" style="padding-top:15px;">
					...
					<button class="left" type="button" onclick="fncGetList()" style="margin-left:0;"><img class="left" src="/images/btn/search.jpg" alt="병원앨범 검색 버튼" /></button>
				</div>
				<div class="list">
			...
			<%
				...
				%>
					<ul class="list_item">
						...
						<li class="title">
							<a href="/mng/contents/hospital_album_detail.asp?key=<%=objRecord("BBS_KEY")%>&dvs=<%=objRecord("BBS_DVS")%>" title="상세정보 페이지로 이동">
								<%=objRecord("TITLE")%>
						...
						<li class="edit">
							<a href="/mng/contents/hospital_album_detail.asp?key=<%=objRecord("BBS_KEY")%>&dvs=<%=objRecord("BBS_DVS")%>" title="수정 페이지로 이동">
			...
			%>				
				...
				<div style="position:relative;">
					<div class="left btn list_back" style="position:absolute; top:5px; right:0;">
						<a class="btn_big blue" href="/mng/contents/hospital_album_detail.asp">새글쓰기</a>
					</div>
