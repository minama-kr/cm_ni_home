...
<head>
	...
	<script>
		...
		function fncEdit(pMode){
			...
			var smrContent = $("#pSmrContent").val();
			if(smrContent == ""){
				alert("요약내용을 입력해 주세요.");
				$("#pSmrContent").focus();
				return false;
	...
	%>	
			...
			%>
				...
				<div class="edit">
					...
					<%
						strSQL = " SELECT COM_CD, COM_NM "&_
						              "     FROM NI_COM_CD_MST   "&_
						              "  WHERE DVS_CD = 'C02' "
						objRecord.Open strSQL, objConn, 1
						Do Until objRecord.EOF
							strSelected = ""
							If strDeptCd = objRecord("COM_CD") Then
								strSelected = "selected='selected'"
							End If
					%>
							<option <%=strSelected%> value="<%=objRecord("COM_CD")%>"><%=objRecord("COM_NM")%></option>
					<%
							objRecord.MoveNext
						Loop
					...
					<div class="top_bd_silver botm_bd_black textarea">
						...
						<textarea id="pSmrContent" name="pSmrContent" rows="5"><%=strSmrCnte%></textarea>
					</div>	
					<div class="top_bd_silver">
				...
				<div class="left btn list_back">
					<a class="btn_big gray" href="/mng/contents/health_sence.asp">목록</a>
				</div>
				<div class="right btn">
