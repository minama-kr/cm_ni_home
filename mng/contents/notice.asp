...
<body id="mng">
	<form id="frm" action="/mng/contents/notice.asp" method="post">
		<!-- 상단 메뉴 -->
			...
			<%
				...
				%>
					<ul class="list_item">
						...
						<li class="title">
							<a href="/mng/contents/notice_detail.asp?key=<%=objRecord("BBS_KEY")%>&dvs=<%=objRecord("BBS_DVS")%>" title="상세정보 페이지로 이동"><%=objRecord("TITLE")%></a>
						</li>
						<li class="view_cnt"><%=objRecord("SRCH_CNT")%></li>
						<li class="edit">
							<a href="/mng/contents/notice_detail.asp?key=<%=objRecord("BBS_KEY")%>&dvs=<%=objRecord("BBS_DVS")%>" title="수정 페이지로 이동">
			...
			%>				
				...
				<div style="position:relative;">
					<div class="left btn list_back" style="position:absolute; top:5px; right:0;">
						<a class="btn_big blue" href="/mng/contents/notice_detail.asp">새글쓰기</a>
					</div>
