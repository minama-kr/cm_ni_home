...
<div id="header" style="border-top:4px solid #b0b0b0; width:100%;">
	<div class="wrap">
		<ul id="info">
			<li style="background:url(/images/top/btn_menu_open.jpg) no-repeat right top; width:103px; height:23px; text-align:right;">
				<a id="site_map_open" class="transparent" href="#site_map_link" onclick="fncLayerPopControl('site_map', true)" title="Site Map 바로가기">전체 메뉴열기</a>
			</li>
		</ul>
	...
	<%
		For i = 0 To UBound(arrM) - 1
			strSelected = ""
			If nSelMenuIndex = i Then
				strSelected = "class='selected'"
			End If
	%>
			<li <%=strSelected%>>
				<a href="<%=arrM(i).url%>"><%=arrM(i).menuNm%></a>
		<% If UBound(arrM(i).arrS) - 1 > 0 AND 1 = 0 Then	%>
				<ul class="sub_nav">
